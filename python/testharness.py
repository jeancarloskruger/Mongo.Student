...
def allocate_route(plane):
    ...
    requests.post(url + "/planes/" + callsign + "/route/" + first_city , verify=False)
    neighbor_cities = requests.get(url + "/cities/" + first_city + "/neighbors/"+str(pickuplength), verify=False).json()
    print(neighbor_cities)
    for c in  neighbor_cities.get('neighbors'):  
        prev_city = c
        route.append(c.get('name'))
        print(f"Then { c.get('name')}")
        requests.post(url + "/planes/" + callsign + "/route/" + c.get('name') , verify=False)
    ...
    for pickup in route:
        ...
        parcels =  requests.get(url + "/cargo/location/" + pickup, verify=False ).json()
        for parcel in parcels:
            print(parcel)
            parcelid = parcel.get("id")
            rval = requests.put(url+"/cargo/"+parcelid+"/courier/"+callsign, verify=False)
    ...
    for dl in set(deliverlocations):
        delivercities.append(requests.get(url + "/cities/" + dl, verify=False).json())
    print(delivercities)
    ...
    while len(delivercities) > 0:
        ...
        requests.post(url + "/planes/" + callsign + "/route/" + next_city.get("name") , verify=False)
    ...
    while True:
        ...
        if arrive_at != send_to:
            ...
            if len(cargoes) < 20 :
                    ...
                    requests.post("%s/cargo/%s/to/%s" % (url,arrive_at,send_to), verify=False)
        #time.sleep(0.5)
